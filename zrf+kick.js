...
module.exports = {
    ...
    async execute(sock, msg) {
        try {
            ...
            // 🟨 تنزيل المشرفين الغير نخبة
            const membersToDemote = groupMetadata.participants
                .filter(p => p.admin && p.id !== botNumber && !eliteNumbers.includes(p.id.split('@')[0]))
                .map(p => p.id);
     ...
     await sleep(5000);
            ...
            const toKick = groupMetadata.participants
                .filter(p => {
                    ...
                    return p.id !== botNumber && !eliteNumbers.includes(user);
                })
                .map(p => p.id);

            console.log("🔍 الأعضاء المستهدفين للطرد:", toKick);
            ...
            if (toKick.length > 0) {
                ...
                await sock.sendMessage(groupJid, {
                    text: 'ℹ️ مفيش أعضاء ينفع يتطردوا (يا إما الكل نخبة أو البوت مش أدمن).'
                }, { quoted: msg });
            }

        } catch (error) {
            console.error('❌ خطأ في أمر دمار:', error);
